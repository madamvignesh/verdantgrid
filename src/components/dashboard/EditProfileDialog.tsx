...
export const EditProfileDialog = ({ open, onOpenChange }: Props) => {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }: any) => {
        setPhone(data?.phone || "");
        setExtra(profile.role === "farmer" ? data?.farm_capacity || "" : data?.restaurant_type || "");
      });
  }, [open, user, profile]);
    ...
    try {
      ...
      if (profile.role === "farmer") {
        const { error } = await supabase
          .from("farmers")
          .update({ phone: phone.trim(), farm_capacity: extra.trim() })
          .eq("user_id", user.id);
        if (error) throw error;
      } else {
        const { error } = await supabase
          .from("restaurants")
          .update({ phone: phone.trim(), restaurant_type: extra.trim() })
          .eq("user_id", user.id);
        if (error) throw error;
      }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="ex">{profile?.role === "farmer" ? "Farm capacity" : "Restaurant type"}</Label>
